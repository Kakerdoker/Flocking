...
public class FirstPersonController : MonoBehaviour
{
    ...
    GUI_handler guiScript;

    private void Awake()
    {
        Physics.gravity = new Vector3(0,-30f,0);
        guiScript = GameObject.Find("Script Holder").GetComponent<GUI_handler>();
    ...
    private void Update()
    {
        if(!guiScript.consoleOn){
            ...
            if(enableJump && Input.GetKeyDown(jumpKey) && jumpAmount > 0)
            {
                Jump();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove && !guiScript.consoleOn)
        {
    ...
    {
        if(isWalking && !guiScript.consoleOn)
        {
